...
function calcularPromedio(listaDeNumeros) {
    ...
    let promedio = sumaTotal / listaDeNumeros.length;
...
console.log("calcularPromedio(): ", calcularPromedio([1,1,1,1]))
...
function filtrarSoloTruthy(listaDeValores) {
    let valoresTruthy = listaDeValores.filter(valor => Boolean(valor))
...
function enumerarLista(listaDePalabras) {
    ...
    oracion += " y " + listaDePalabras.slice(-1);
    return oracion.charAt(0).toUppercase() + oracion.slice(1) + " y "
    // slice devuelve una copia de una parte de array dentro de un nuevo array, empezando por incio hasta fin (fin no incluido), el array original no se modifica
}
